...
def create_account():
    ...
    def submit():
        if len(first_name.get()) != 0 and len(last_name.get()) != 0 and len(username.get()) != 0 and len(
            ...
            cmd = "insert into bankers (acc_no,fname,lname,username,password,city,balance) values (%s,%s,%s,%s,%s,%s,%s)"
            cur.execute(cmd, params=[acc_no,str(first_name.get()), str(last_name.get()),str(username.get()), str(passwd.get()), str(city.get()),0])
            if cur.rowcount==1:
                conn.commit()
...
def delete_acc():
    ...
    def delete():
        acc_no=accNum.get()
        cur.execute("delete from bankers where acc_no=%s",params=[acc_no])
        if cur.rowcount==1:
            conn.commit()
...
def login_user():
    ...
    def submit():
        uname=username.get()
        pwd=passwd.get()
        cur.execute("select * from bankers where username=%s and password=%s",params=[uname,pwd])
        data=cur.fetchone()
        if cur.rowcount==1:
            conn.commit()
        if data==None:
...
account=tk.Button(window,text="Create Account",width=13,font=("Arial",12),bg="blue",fg="white",command=create_account)
login=tk.Button(window,text="Login Account",width=13,font=("Arial",12),bg="blue",fg="white",command=login_user)
delete=tk.Button(window,text="Delete existing account",width=20,font=("Arial",12),bg="blue",fg="white",command=delete_acc)
